 ...
 */
function sleekblog_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'sleekblog_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'sleekblog' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => sleekblog_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'sleekblog' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sleekblog' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'sleekblog' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sleekblog' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'sleekblog' );
}
add_action( 'customize_register', 'sleekblog_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'sleekblog_color_schemes'}.
 *
 ...
 */
function sleekblog_get_color_schemes() {
	return apply_filters( 'sleekblog_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'sleekblog' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'sleekblog_get_color_scheme' ) ) :
/**
 ...
 */
function sleekblog_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = sleekblog_get_color_schemes();
...
endif; // sleekblog_get_color_scheme

if ( ! function_exists( 'sleekblog_get_color_scheme_choices' ) ) :
/**
 ...
 */
function sleekblog_get_color_scheme_choices() {
	$color_schemes                = sleekblog_get_color_schemes();
...
endif; // sleekblog_get_color_scheme_choices

if ( ! function_exists( 'sleekblog_sanitize_color_scheme' ) ) :
/**
 ...
 */
function sleekblog_sanitize_color_scheme( $value ) {
	$color_schemes = sleekblog_get_color_scheme_choices();
...
endif; // sleekblog_sanitize_color_scheme
 ...
 */
function sleekblog_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = sleekblog_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = sleekblog_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = sleekblog_get_color_scheme_css( $colors );

	wp_add_inline_style( 'sleekblog-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'sleekblog_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function sleekblog_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', sleekblog_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'sleekblog_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function sleekblog_customize_preview_js() {
	wp_enqueue_script( 'sleekblog-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'sleekblog_customize_preview_js' );
 ...
 */
function sleekblog_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function sleekblog_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-sleekblog-color-scheme">
		<?php echo sleekblog_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'sleekblog_color_scheme_css_template' );
